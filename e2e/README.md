...
A real Convex reactive client driven against a real Convex deployment — natively
(control) and through the SSE proxy — to confirm live queries resolve over the
tunnel. It exercises the built client bundle, so build first.
...
The first outbound upgrade after `wrangler dev` cold-starts can need a couple of
retries (workerd warming up); Convex's reconnect recovers in ~1–3s.
